...
**Last Updated:** 2026-06-08 — **Phase 1 (Session 3) — step 3 (chunk) DONE + eval methodology locked.** `src/rag_exp/chunk.py` written (recursive token-accurate splitter → `Chunk` records; stable ids `product_id:source:ordinal`; metadata inherited), ruff-clean, smoke-tested: **990 chunks** (detective 317 · science 563 · shakespeare 110), token len min 6 / max 510 / mean 396, **0 over the 512 cap**, ids unique & all stamped. Explainers: chunking real-counts + tuning-knobs, new `embedding.md`. **ADR-004** locked the eval design — golden set anchored to **source text/quotes** (not chunk ids, so it survives sweeps) + a **cost-tiered sweep** methodology — realigning spec H.1/Part I + the chunking-lab/rag-eval-harness skills + chunking explainer off chunk-id pinning. **Step 4 (embed) is next.** _Prior:_ steps 1–2 (env + corpus + `config` + `ingest`, Session 2), Phase 0 scaffolding (Session 1).
